...
| H4 | 1.4.3 Contrast (Minimum) | `src/data.ts` all themes | Every theme | Muted-foreground text and non-text UI component contrast needed verification across all 6 themes (esp. `whitecoat`, `sunset`). See `theme-contrast.md`. | Documented measured ratios; flagged any sub-4.5:1 body / sub-3:1 UI pairs for palette tuning (human sign-off required before changing brand colors). | Partial (tooling measures; palette change needs approval) |
