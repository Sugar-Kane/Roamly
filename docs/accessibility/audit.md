...
| M2 | 1.1.1 / 1.3.1 | `Charts.tsx`, `StudyInsights.tsx` | Analytics | Recharts SVG charts were the only way to read the data; pointer-only tooltips. | **RESOLVED** — `WeekChart` and all three admin BI charts carry visually-hidden data tables (via a shared `ChartTable` helper); the `SubjectDonut` SVG is hidden from AT with its existing visible name+percentage list as the text alternative. `StudyInsights` CSS bars already show text labels+values. | Yes |
